<div class="flex flex-col">
                ...
                @foreach ($barangJadi as $row)
                    ...
                    @foreach ($modelBarangJadi as $item)
                        ...
                        <ul class="container mx-auto divide-y divide-gray-400 divide-dotted">
                            <li class="grid items-center w-full grid-cols-12 gap-2 py-2 w-full">
                                {{--  akhir aktion model  --}}
                                <div class="col-span-11 px-6">
                                            ...
                                            @foreach ($warnaModel as $indexWarna => $rowItem)
                                                ...
                                                <tr>
                                                    ...
                                                    <td class="px-4 py-2 text-xs">
                                                        {{ \Carbon\Carbon::parse($rowItem?->updated_at)?->setTimezone('Asia/Jakarta')->translatedFormat('l, d F Y H:i:s') }}
                                                    </td>
                                                    <td class="px-4 py-2 text-xs">{{ $rowItem?->jumlah }}
                                                        (Pcs)
                                                    </td>
                                                    <td class="px-4 py-2 text-xs">
                                                        {{ formatRupiah($rowItem?->harga) }}
                                                        /pcs
                                                    </td>
                                                    <td class="px-4 py-2 text-xs">
                                                        {{ formatRupiah($rowItem?->total) }}
                                                    </td>
    ...
    <script>
        ...
        $(document).on('click', '.edit-tanggal, .edit-warna, .edit-model', function(e) {
            ...
            if ($(this).hasClass('edit-tanggal')) {
                ...
                postUrl = `/barang/jadi/warna/update`;
                ...
                `;
            } else if ($(this).hasClass('edit-model')) {
                url = `/barang/jadi/model/edit/${post_id}`;
                postUrl = `/barang/jadi/model/update`;
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (modalTitle === 'Edit Tanggal') {
                        $('#tanggal_kirim').val(formatDateTime(response.data
                            .tanggal_kirim));
