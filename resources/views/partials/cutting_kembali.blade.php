<div class="flex flex-col">
            ...
            <div class="border divide-y divide-gray-200 rounded-lg dark:divide-neutral-700 dark:border-neutral-700">
                ...
                @foreach ($cuttingAmbil as $row)
                    ...
                    <div
                        ...
                        Tanggal:
                        {{ \Carbon\Carbon::parse($row?->tanggal_kirim)?->setTimezone('Asia/Jakarta')->translatedFormat('l, d F Y H:i:s') }}
                                                    ...
                                                    <td class="px-4 py-2 text-xs">
                                                        <div class="flex items-center">
                                                            <input type="number" name="jumlah_kembali"
                                                                ...
                                                                value="{{ $rowItem?->jumlah_kembali }}">
                                                            <p class="ps-2">(Pcs)</p>
                                                        </div>
                                                    </td>
                                                    <td class="px-4 py-2 text-xs total_ongkos">
                                                        {{ formatRupiah($rowItem?->total_ongkos) }}
                                                    </td>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('input[name="jumlah_kembali"], input[name="tanggal_kembali"]').on('input', function(e) {
                ...
                timeoutId = setTimeout(function() {
                    $.ajax({
                        ...
                        data: {
                            _token: '{{ csrf_token() }}',
                            jumlah_kembali: inputName === 'jumlah_kembali' ? value :
                                undefined,
                            tanggal_kembali: inputName === 'tanggal_kembali' ? value :
                                undefined
                        },
                        success: function(response) {
                            console.log(response)
                            $('.total_ongkos').html(formatNominal(response.warna
                                .total_ongkos));
                            Toast.fire({
                                icon: "success",
