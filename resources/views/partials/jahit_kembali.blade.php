        ...
        <div class="inline-block min-w-full p-1.5 align-middle">
            <div
                ...
                @foreach ($jahitAmbil as $row)
                    ...
                    @endphp
                    @php
                        $cutingModel = \App\Models\JahitAmbilModel::getReturnNull($row?->id);
                    @endphp
                                        ...
                                        <tbody class="bg-white divide-y divide-purple-500">
                                            @foreach ($warnaModel as $indexWarna => $rowItem)
                                                <tr>
                                                    ...
                                                    @php
                                                        $isitReturn = \App\Models\JahitKembali::isitReturn(
                                                            $rowItem?->id,
                                                        );
                                                    @endphp
                                                    @if ($isitReturn == null)
                                                    ...
                                                    @else
                                                        @php
                                                            ...
                                                            $totalKeseluruhanHarga +=
                                                                $getJahitWarnaModel?->total_ongkos;
                                                            $getGajiByWarnaJahit = App\Models\Gaji::getGajiByWarnaJahit(
                                                                $getJahitWarnaModel?->id,
                                                            );
                                                        @endphp
                                                        <td class="px-4 py-2 text-xs">
                                                            {{ $getJahitWarnaModel?->tanggal_kembali }}
                                                        </td>
                                                        <td class="px-1 py-2 text-xs">
                                                            {{ $getJahitWarnaModel?->jumlah_kembali }}
                                                            ({{ $getJahitWarnaModel?->satuan_kembali }})
                                                        </td>
                                                        <td class="px-4 py-2 text-xs total_ongkos">
                                                            {{ formatRupiah($getJahitWarnaModel?->total_ongkos) }}
                                                        </td>
                                                        <td class="text-center">
                                                            <div
                                                                class="flex items-center justify-center py-2 hs-tooltip">
                                                                <button
                                                                    @if ($getGajiByWarnaJahit?->status == 'belum terbayarkan' || $getGajiByWarnaJahit?->status === 'terbayarkan') data-id="{{ $getGajiByWarnaJahit?->id }}"
                                                                        data-ongkos="{{ formatNominal($getGajiByWarnaJahit?->nominal_belum_terbayarkan) }}" @endif
                                                                    class="{{ $getGajiByWarnaJahit?->status === 'lunas' ? 'bg-green-500' : ($getGajiByWarnaJahit?->status === 'belum terbayarkan' ? 'bg-red-500 bayarBtn' : 'bg-yellow-500 bayarBtn') }} hs-tooltip-toggle text-nowrap inline-flex capitalize items-center gap-x-1.5 rounded-full px-3 py-1.5 text-xs font-medium text-white">
                                                                    {{ $getGajiByWarnaJahit?->status }}
                                                                    @if ($getGajiByWarnaJahit->nominal_belum_terbayarkan > 0)
                                                                        <span role="tooltip"
                                                                            ...
                                                                            {{ formatRupiah($getGajiByWarnaJahit->nominal_belum_terbayarkan) }}
                    ...
                    @endforeach
                    @php
                        $BonJahit = App\Models\Bon::getBonJahit($karyawan->id, $row->id);
                        $GajiJahit = App\Models\Gaji::getGajiJahit($karyawan->id, $row->id);
                    @endphp
                    <div
        ...
        $(document).ready(function() {
            ...
            $(document).on('click', '.kembaliBtn', function(e) {
                ...
                if ($(this).hasClass('kembaliBtn')) {
                    postUrl = `/karyawan/jahit/kembali/{{ $karyawan->id }}/${post_id}/store`;
                    modalTitle = 'Pengembalian Jahit';
                    modalContent = `
            ...
            $(document).on('click', '.bayarBtn', function(e) {
                ...
                postUrl = `/karyawan/jahit/gaji/status`;
