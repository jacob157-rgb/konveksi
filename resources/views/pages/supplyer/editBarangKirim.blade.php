...
@section('content')
    <div
        ...
        <h2 class="font-bold text-center text-white uppercase">ID : {{ $barangKirimFirst->unique_id }}</h2>
        <h2 class="font-bold text-center text-white uppercase">TANGGAL :
            {{ \Carbon\Carbon::parse($barangKirimFirst->tanggal_kirim) }}</h2>
    </div>
    <div
        ...
        <div class="flex flex-col">
            <div class="-m-1.5 overflow-x-auto">
                <form action="/barang/jadi/update/{{ $barangKirimFirst->id }}" method="post" id="updateBarangKirimForm">
                    @csrf
                    @method('PUT')
                    <div class="inline-block min-w-full py-1.5 align-middle">
                        ...
                        <input type="hidden" name="unique_id" value="{{ $barangKirimFirst->unique_id }}">
                        <input type="text" name="supplyer_id" hidden value="{{ $barangKirimFirst->supplyer_id }}">
                        <div class="px-3 py-4 overflow-y-auto model-container">
                            ...
                            <input type="datetime-local" id="tanggal_kirim" name="tanggal_kirim"
                                value="{{ \Carbon\Carbon::parse($barangKirimFirst->tanggal_kirim)->format('Y-m-d\TH:i') }}"
                                ...
                                autofocus="">
                            <!-- Unique ID -->
                            <label for="unique_id" class="block pt-2 mb-2 text-sm font-medium dark:text-white">Id Barang
                                Mentah</label>
                            <input type="text" id="unique_id" name="unique_id" value="{{ $barangKirimFirst->unique_id }}"
                                class="block w-full px-4 py-3 text-sm border border-gray-200 rounded-lg dark:border-neutral-700 dark:bg-neutral-900 dark:text-neutral-400 dark:placeholder-neutral-500 focus:border-blue-500 focus:ring-blue-500"
                                autofocus="" disabled>
                            <input type="hidden" name="unique_id" value="{{ $barangKirimFirst->unique_id }}">
                            <!-- End Unique ID -->

                            @foreach ($barangKirimFirst->modelBarangJadi as $index => $modelBarang)
                                <div class="model-card">
                                    <input type="hidden" name="model[{{ $index }}][id]"
                                        value="{{ $modelBarang->id }}">
                                    <div
                                        ...
                                        <select name="model[{{ $index }}][nama]"
                                            class="block w-full px-4 py-3 text-sm border border-gray-200 rounded-lg dark:border-neutral-700 dark:bg-neutral-900 dark:text-neutral-400 dark:placeholder-neutral-500 focus:border-blue-500 focus:ring-blue-500">
                                            <option value="{{ $modelBarang->model }}" selected>{{ $modelBarang->model }}
                                            </option>
                                        ...
                                        @foreach ($modelBarang->warnaModel as $warnaIndex => $warnaModel)
                                            <div class="warna-container">
                                                <div class="warna-card">
                                                    <input type="hidden"
                                                        name="model[{{ $index }}][warna][{{ $warnaIndex }}][id]"
                                                        value="{{ $warnaModel->id }}">
                                                    <div
                                                        ...
                                                        <div class="value-container">
                                                            <label
                                                                for="jumlah_jadi_{{ $index }}_{{ $warnaIndex }}"
                                                                class="block mb-2 text-sm font-medium dark:text-white">Jml.
                                                                Barang</label>
                                                            <div class="relative">
                                                                <input type="number"
                                                                    id="jumlah_jadi_{{ $index }}_{{ $warnaIndex }}"
                                                                    name="model[{{ $index }}][warna][{{ $warnaIndex }}][jumlah_jadi]"
                                                                    value="{{ $warnaModel->jumlah }}"
                                                                    class="block w-full px-4 py-3 text-sm border border-gray-200 rounded-lg shadow-sm dark:border-neutral-700 jumlah dark:bg-neutral-900 dark:text-neutral-400 dark:placeholder-neutral-500 dark:focus:ring-neutral-600 focus:border-blue-500 focus:ring-blue-500">
                                                                <div
                                                                    class="absolute inset-y-0 flex items-center text-gray-500 end-0 pe-px">
                                                                    <label
                                                                        for="satuan_{{ $index }}_{{ $warnaIndex }}"
                                                                        class="sr-only">Satuan</label>
                                                                    <select
                                                                        id="satuan_{{ $index }}_{{ $warnaIndex }}"
                                                                        name="model[{{ $index }}][warna][{{ $warnaIndex }}][satuan]"
                                                            ...
                                                            <label for="harga_{{ $index }}_{{ $warnaIndex }}"
                                                                class="block mb-2 text-sm font-medium dark:text-white">Harga
                                                                Barang</label>
                                                            <div class="relative rounded-md">
                                                                <input type="text"
                                                                    name="model[{{ $index }}][warna][{{ $warnaIndex }}][harga]"
                                                                    value="{{ $warnaModel->harga }}"
                                                                    class="block w-full px-4 nominal price py-3 text-sm border-gray-200 rounded-lg shadow-sm dark:border-neutral-700 dark:bg-neutral-900 dark:text-neutral-400 dark:placeholder-neutral-500 dark:focus:ring-neutral-600 pe-16 ps-10 focus:border-blue-500 focus:ring-blue-500">
                                                                <div
                                                                    class="absolute inset-y-0 z-20 flex items-center pointer-events-none start-0 ps-4">
                                                                    <span
                                                                        class="text-gray-500 dark:text-neutral-500">Rp.</span>
                                                                </div>
                                                                <div
                                                                    class="absolute inset-y-0 z-20 flex items-center pointer-events-none end-0 pe-4">
                                                                    <span
                                                                        class="text-gray-500 dark:text-neutral-500">IDR</span>
                                                                </div>
                                                            </div>

                                                            <label for="total_{{ $index }}_{{ $warnaIndex }}"
                                                                class="block mb-2 text-sm font-medium dark:text-white">Total
                                                                Harga</label>
                                                            <div class="relative rounded-md">
                                                                <input type="text" readonly
                                                                    name="model[{{ $index }}][warna][{{ $warnaIndex }}][total]"
                                                                    value="{{ $warnaModel->total }}"
                                                                    class="block w-full px-4 total price py-3 text-sm border-gray-200 rounded-lg shadow-sm dark:border-neutral-700 dark:bg-neutral-900 dark:text-neutral-400 dark:placeholder-neutral-500 pe-16 ps-10 focus:border-blue-500 focus:ring-blue-500">
                                                                <div
                                                                    class="absolute inset-y-0 z-20 flex items-center pointer-events-none start-0 ps-4">
                                                                    <span
                                                                        class="text-gray-500 dark:text-neutral-500">Rp.</span>
                    ...
                    <div class="flex items-center justify-between px-4 pt-3 border-t dark:border-neutral-700 gap-x-2">
                        <button type="button"
                            class="inline-flex items-center px-3 py-2 text-sm font-medium text-gray-800 bg-white border border-gray-200 rounded-lg shadow-sm dark:border-neutral-700 dark:bg-neutral-900 dark:text-white dark:hover:bg-neutral-800 gap-x-2 hover:bg-gray-50">
                            Batal
                        </button>
                        <button type="button" id="submitFormBtn"
                            class="inline-flex items-center px-3 py-2 text-sm font-semibold text-white bg-blue-600 border border-transparent rounded-lg gap-x-2 hover:bg-blue-700">
                            Update
                        </button>
                    </div>
                </form>
                        ...
                        <div>
                            <ul class="space-y-3 text-sm">
                                ...
                                @foreach ($items as $it => $item)
                                    <ul
                                        class="text-sm font-semibold {{ $barangKirimFirst->id == $item->id ? 'bg-amber-200 p-2 rounded' : '' }}">
                                        @foreach ($item->modelBarangJadi as $model)
                                            <li class="flex gap-x-3">
                                                ...
                                                <span class="text-gray-800 dark:text-neutral-400">
                                                    <span class="text-green-600"> {{ $model->model }} </span>
                                                    -
                                                ...
                                                <table
                                                    ...
                                                    <tbody>
                                                        @foreach ($model->warnaModel as $model)
                                                            @php $totalSemua += $model->total; @endphp
                                                            <tr>
                ...
                <div
                    ...
                    <table class="w-full text-left table-auto min-w-max">
                        ...
                        <tbody>
                            @foreach ($items as $it => $item)
                                <tr
                                    class="hover:bg-slate-50 {{ $barangKirimFirst->id == $item->id ? 'bg-amber-200 p-2 rounded' : '' }}">
    ...
    <script>
        document.getElementById('submitFormBtn').addEventListener('click', function() {
            var form = document.getElementById('updateBarangKirimForm');
            var formData = new FormData(form);

            Swal.fire({
                title: 'Proses..',
                text: 'Proses update barang kirim.',
                didOpen: () => {
                    Swal.showLoading();
                },
                ...
                .then(data => {
                    if (data.success) {
                        Swal.fire({
                            icon: 'success',
                            title: 'Success',
                            text: 'Barang kirim berhasil diupdate.',
                        }).then(() => {
