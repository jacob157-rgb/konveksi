...
@section('content')
    <div class="inline-block min-w-full p-1.5 align-middle">
        <div class="p-4 bg-white shadow rounded-xl dark:bg-neutral-900 sm:p-7">
            <h1 class="mb-4 text-2xl font-bold">Edit Pengeluaran</h1>
            <form action="/pengeluaran/update/{{ $pengeluaran->id }}" method="POST">
                @csrf
                <div class="mt-2 space-y-3">
                    <label for="harga" class="block text-sm font-medium text-gray-700 dark:text-neutral-400">Nominal</label>
                    <div class="relative border rounded-md">
                        <input type="text" id="harga" name="nominal" value="{{ formatNominal($pengeluaran->nominal) }}"
                ...
                <div class="mt-2 space-y-3">
                    <label for="keterangan" class="block text-sm font-medium text-gray-700 dark:text-neutral-400">Keterangan</label>
                    <textarea name="keterangan"
                        ...
                        rows="3" placeholder="Tuliskan Keterangan Pengeluaran disini.">{{ $pengeluaran->keterangan }}</textarea>
                </div>
